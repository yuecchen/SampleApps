...
#FILE = open(FILENAME, 'w')
	...
	for x in range(1, MAX_ID):
		...
		if album_count < MAX_ALBUMID:
			...
			album_count = 1
			album_id += 1
		data_string +='   "albumId": '+str(album_id)+',\n'
		data_string +='   "Id": '+str(x)+',\n'
		data_string +='   "title": "'+sentence_maker()+'",\n'
		data_string +='   "url": "http://placeholder.org/'+str(album_id)+'00/'+random_alphanum()+'",\n'
		data_string +='   "thumbnailUrl": "http://placeholder.org/'+str(album_id)+'00/'+random_alphanum()+'.jpg"\n'
		if x < MAX_ID-1:
			data_string +="  },\n"
		else:
			data_string +="  }\n"
...
print(generate_json_data(MAX_ALBUMID, MAX_ID))
